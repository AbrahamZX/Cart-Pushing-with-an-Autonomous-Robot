...
class RRT:
        ...
        if(testgoal<=goal_bias):                                         #occasionally returns the goal node as the new point, rate is controlled by goal_bias
            ...
            if(self.replan==False):
                ...
                while(True):                                      #While loop runs until we find viable coordinates for the second point
                    gaus_x=int(np.round(np.random.normal(rand_x,50)))  #generating points randomly from a gaussian/normal distribution for x and y
                    gaus_y=int(np.round(np.random.normal(rand_y,50)))
    ...
    def RRT_star(self, n_pts=1000, neighbor_size=20,v=[],prevmap=[]):
        '''RRT* search function
        arguments:
        ...
        if self.found:
            ...
            ps=self.draw_map()
            smoother_start_time = time.time()
            new_path = self.rrn(ps)
            [bez_row, bez_col]=self.bezier_smoothing(new_path, mtr=50)
            print("Smoother Runtime is %s seconds" % (time.time() - smoother_start_time))
    ...
    def replanner(self,path,vertices,prevmap):
        ...
        if(flag==1):
            ...
            self.RRT_star(200,50,valid,prevmap)
